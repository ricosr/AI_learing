...
def RNN(X, weights, biases):     # https://zhuanlan.zhihu.com/p/28919765
    ...
    # final_state是最后的(c状态, h状态), c就是和h并列的另一个循环的部件, 可以理解成 LSTM 隐层由两部分组成，一部分叫 h，另一部分叫 c 就行了
    ...
    # transpose交换维度变为[训练次数, 一个批次个数, 神经元个数]:[28,128,120], unstack默认axis为0, 即将第一个维度拆开成28个
...
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=y))    # y:[120, 10]
