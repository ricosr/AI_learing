...
def load_img(path):   # ??????
    ...
    img = img / 255.0    # 调整像素值0~1
    # print "Original Image Shape: ", img.shape
    # we crop image from center
    short_edge = min(img.shape[:2])    # 取较小一边的值, shape是长度为元组, shape[:2]取前两个维度
    yy = int((img.shape[0] - short_edge) / 2)
    xx = int((img.shape[1] - short_edge) / 2)
    crop_img = img[yy: yy + short_edge, xx: xx + short_edge]   # 裁剪图片大小, why??????
    # resize to 224, 224
...
def load_data():
    ...
    for k in imgs.keys():
        ...
        for file in os.listdir(dir):
            if not file.lower().endswith('.jpg'):
                continue
            try:
                resized_img = load_img(os.path.join(dir, file))   # ./for_transfer_learning/data/tiger(or kittycat)/temp.jpg
            except OSError:
                continue
    ...
    tigers_y = np.maximum(20, np.random.randn(len(imgs['tiger']), 1) * 30 + 100)    # 老虎长度
    cat_y = np.maximum(10, np.random.randn(len(imgs['kittycat']), 1) * 8 + 40)      # 猫长度
    # In [3]: np.maximum(20, [1, 3, 60, 90, 7, 20])
    # Out[3]: array([20, 20, 60, 90, 20, 20])
...
class Vgg16:
    ...
    def __init__(self, vgg16_npy_path=None, restore_from=None):
        ...
        self.fc6 = tf.layers.dense(self.flatten, 256, tf.nn.relu, name='fc6')    # 全连接层
...
def train():
    ...
    xs = np.concatenate(tigers_x + cats_x, axis=0)    # tigers_x和cats_x是两个列表, WTF!!!!!!!!!!
    ys = np.concatenate((tigers_y, cats_y), axis=0)

    vgg = Vgg16(vgg16_npy_path='./for_transfer_learning/vgg16.npy')
    print('Net built')
    for i in range(100):
        b_idx = np.random.randint(0, len(xs), 6)    # 一个batch取6个图片数据
