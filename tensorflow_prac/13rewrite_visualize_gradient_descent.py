...
a3D, b3D = np.meshgrid(np.linspace(-2, 7, 30), np.linspace(-2, 7, 30))  # parameter space 将两个一维数组变为二维矩阵, 用两个坐标轴上的点在平面上画格
# https://www.cnblogs.com/sunshinewang/p/6897966.html
cost3D = np.array([np.mean(np.square(y_fun(a_, b_) - y)) for a_, b_ in zip(a3D.flatten(), b3D.flatten())]).reshape(a3D.shape)
# z轴cost对应xy网格的点的值
ax.plot_surface(a3D, b3D, cost3D, rstride=1, cstride=1, cmap=plt.get_cmap('rainbow'), alpha=0.9)    # 绘面, rstride=1(y), cstride=1(x) 取值步长, alpha 透明度
ax.scatter(a_list[0], b_list[0], zs=cost_list[0], s=100, c='r')  # initial parameter place  s:点所在区域大小
ax.set_xlabel('a'); ax.set_ylabel('b')
ax.plot(a_list, b_list, zs=cost_list, zdir='z', c='r', lw=3)    # plot 3D gradient descent, zdir:哪个坐标轴最为z轴, lw线宽度
plt.show()
