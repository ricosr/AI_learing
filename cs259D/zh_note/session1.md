...
### 例子: 使用Netflow建模进行P2P僵尸网络检测
- 图中每个节点表示一台主机(IP 地址)
- 边(A, B)有且仅有一条线路从A到B
- 计算Hubs and Authorities分数
- 僵尸机器人会显示相似hub-authority的特征
- 在二维空间聚类(hub, authority)
...
- 用于检测特定偏差的异常传感器
   ...
   - 每个用户登录的机器的配置文件集
   - 模拟每个用户的正常工作时间
   - 建立内部主机之间数据流的模型 (例如: 检测分段的服务器在数据渗透之前)
